...
class RelocateEnvV0(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def _step(self, a):
        ...
        ob = self._get_obs()
        reward = self.get_reward(ob,a)

        score = self.get_score(ob)
        env_info = {'score': score}

        return ob, reward, False, env_info
    ...
    def get_reward(self, observations, actions):
        ...
        if observations.ndim == 1:
            palm_pos  = observations[30:33]
            obj_pos = observations[33:36]
            target_pos = observations[36:39]
            ...
            assert observations.ndim == 2  # for the planner to select actions
            ...
            reward = -0.1*np.linalg.norm(palm_pos-obj_pos, axis=1)  # take hand to object

            index = np.array([i for i in range(n)])
            Idx = index[(obj_pos[:,2] > 0.04)]                     # if object off the table
            reward[Idx] += 1.0                                     # bonus for lifting the object

            reward += -0.5*np.linalg.norm(palm_pos-target_pos, axis=1)      # make hand go to target
            reward += -0.5*np.linalg.norm(obj_pos-target_pos, axis=1)       # make object go to target

            Idx = index[(np.linalg.norm(obj_pos-target_pos) < 0.1)]   # if object off the table
            reward[Idx] += 10.0                                       # bonus for object close to target
            Idx = index[(np.linalg.norm(obj_pos-target_pos) < 0.05)]  # if object off the table
            reward[Idx] += 20.0                                       # bonus for object close to target
    ...
    def _get_obs(self):
        ...
        # print('qp', qp)
        # return np.concatenate([qp[:-6], palm_pos-obj_pos, palm_pos-target_pos, obj_pos-target_pos])   # the paper uses this observations
        return np.concatenate([qp[:-6], palm_pos, obj_pos, target_pos])
    ...
    def ss(self, state_dict):
        ...
        # palm_pos = state_dict['palm_pos']  ## set palm ... added by ray
        self.set_state(qp, qv)
        self.model.body_pos[self.obj_bid] = obj_pos
        self.model.site_pos[self.target_obj_sid] = target_pos
        # self.model.site_pos[self.S_grasp_sid] = palm_pos   ## set palm ... added by ray
        self.sim.forward()
